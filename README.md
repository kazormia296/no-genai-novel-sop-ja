...
> 本プロジェクトは、SOP `1.2.1`、タグ `sop-v1.2.1`、コミット `<40桁SHA>`、SOP文書SHA-256 `<64桁ハッシュ>` を基準版として採用し、基礎プロファイル `<P2／P3>` と追加プロファイル `P4-H` に従って運用した。監査保証区分は `<SELF-ATTESTED／SECOND-PARTY／INDEPENDENT>`、最終結果は `<PASS／CONDITIONAL PASS>` であり、ローカル修正は `<なし／識別子>` である。
